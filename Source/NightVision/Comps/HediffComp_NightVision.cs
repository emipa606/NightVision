// Nightvision HediffComp_NightVision.cs
// 
// 16 05 2018
// 
// 21 07 2018
...
{
    [UsedImplicitly] public HediffCompProperties_NightVision Props => (HediffCompProperties_NightVision)props;
    ...
    private string TipString()
    {
        switch (Props.LightModifiers.Setting)
        {
            ...
            case VisionType.NVPhotosensitivity:
                return "NVGivePS".Translate();
            case VisionType.NVCustom:
                var result = new StringBuilder();
                if (Math.Abs(Props.LightModifiers[0]) > 0.0001)
                {
                    result.AppendFormat("{0} = {1:+#;-#;0}% {2}", "NVZeroSh".Translate(), Props.LightModifiers[0] * 100,
                        "NVWorkMoveShort".Translate());
                ...
                if (Math.Abs(Props.LightModifiers[1]) > 0.0001)
                {
                    result.AppendFormat("{0} = {1:+#;-#;0}% {2}", "NVFullSh".Translate(), Props.LightModifiers[1] * 100,
                        "NVWorkMoveShort".Translate());
                }
