// Nightvision Comp_NightVision.cs
// 
// 16 05 2018
// 
// 21 07 2018
...
public class Comp_NightVision : ThingComp
{
    ...
    public List<Apparel> PawnsNVApparel = [];
    ...
    public override void Initialize(CompProperties compprops)
    {
        ...
        if (parent == null || ParentPawn is not { Spawned: true } || ParentPawn.health == null || ParentPawn.Dead)
        {
            return;
        }
                         ...
                         && PawnsNVHediffs[part.Label].Remove(hediff.def))
        {
            if (part.def.tags.Contains(Defs_Rimworld.EyeTag)
                && (hediff is Hediff_MissingPart
                    || hediff.def.addedPartProps is { solid: true }))
            {
        ...
        BodyPartRecord part = null)
    {
        ...
        if (part != null)
        {
            ...
            if (PawnsNVHediffs.TryGetValue(partName, out var tempPartsHediffDefs))
            {
                ...
                {
                    removedPart = true;
                    tempPartsHediffDefs = [HediffDefOf.MissingBodyPart];
                }
                else
                {
                    ...
                    {
                        if (hediffDef.addedPartProps is { solid: true })
                        {
                            removedPart = true;
                            tempPartsHediffDefs = [hediffDef];
                        }
                    ...
                    else if (hediffDef.addedPartProps is { solid: true })
                    {
                        removedPart = true;
                        tempPartsHediffDefs = [hediffDef];
                    }
            ...
            {
                PawnsNVHediffs[partName] = [hediff.def];
            }
        }
        else if (allSightHediffs.Contains(hediff.def))
        {
    ...
    private void InitPawnsHediffsAndCountEyes()
    {
        ...
        {
            PawnsNVHediffs[RaceSightParts[i].Label] = [];
        }

        PawnsNVHediffs[Str.BodyKey] = [];
    ...
    private void InitPawnsApparel()
    {
        ...
        if (ParentPawn.apparel?.WornApparel is not { } pawnsApparel)
        {
            return;
        }
