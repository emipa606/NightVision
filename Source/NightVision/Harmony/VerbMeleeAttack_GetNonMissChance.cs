...
public static class VerbMeleeAttack_GetNonMissChance
{
    ...
    {
        if (__result > 0.999f || __instance.CasterPawn is not { } pawn ||
        ...
        var glow = pawn.Map != null ? GlowFor.GlowAt(pawn.Map, target.Cell) : Constants.TRIVIAL_GLOW;
        ...
        if (target.Thing is Pawn t_pawn)
        {
            ...
            if (CurrentStrike.GlowDiff.IsNonTrivial() && CurrentStrike.SurpAtkSuccess)
            {
                ...
                __result = 1f;
                MoteMaker.ThrowText(pawn.Position.ToVector3Shifted(), pawn.Map, "NightVisionSneakAtkMote".Translate(),
                    5f);
