...
public static class JobGiverAIFightEnemy_FindAttackTarget
{
    ...
    public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        var callNVModifyRadius = new CodeInstruction(OpCodes.Call,
            AccessTools.Method(typeof(AICombatTweaks), nameof(AICombatTweaks.ModifyTargetAcquireRadiusForGlow)));
        ...
        foreach (var instruction in instructions)
        {
            ...
            else if (ldfldPawn == null && instruction.opcode == OpCodes.Ldfld && instruction.operand is FieldInfo fi &&
                     fi.FieldType == typeof(Pawn))
            {
                // .pawn
