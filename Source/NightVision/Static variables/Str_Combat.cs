...
public static class Str_Combat
{
    ...
    public static string HitChanceHeader()
    {
        return
            $"{$" {"NightVisionTarget".Translate()}:   0% {"Lit".Translate()}".PadLeft(40, NumSpace)}  |  100% {"Lit".Translate()}";
    }
    ...
    {
        return
            $"{$"{hitChance.ToStringPercent("F1")}  ==>".PadLeft(28, NumSpace)}{nvResult.ToStringPercent("F1").PadLeft(7, NumSpace)}  |  {psResult.ToStringPercent("F1")}";
    }
...
#else
        return
            $"{"NightVisionOurLM".Translate().RawText.PadLeft(20, NumSpace)}  |{"NightVisionTargetLM".Translate().RawText.PadLeft(12, NumSpace)}  |{"NightVisionSurpriseAtkSh".Translate().RawText.PadLeft(10, NumSpace)}";
#endif
    }
...
#else
        return
            $"{"NightVisionOurLM".Translate().RawText.PadLeft(20, NumSpace)}  |{"NightVisionAtkLM".Translate().RawText.PadLeft(12, NumSpace)}  |{"NightVisionDodgeChanceShort".Translate().RawText.PadLeft(10, NumSpace)}";
#endif
    }
    ...
    {
        return "NightVisionAimFactorFromLight".Translate(glowAtTarget.ToStringPercent(), result.ToStringPercent())
            .CapitalizeFirst();
    }

    public static string RangedCooldown(float glow, int skill, float result)
    {
        return "NightVisionRangedCooldown".Translate(glow.ToStringPercent(), skill, result.ToStringPercent());
    }

    public static string RangedCooldownDemo(float glow, float result)
    {
        return "NightVisionRangedCooldownDemo".Translate(glow.ToStringPercent(), result.ToStringPercent());
    }
}